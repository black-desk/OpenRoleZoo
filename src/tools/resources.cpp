  ...
  namespace resources
  {
    ...
    class resources_hash_node {
      public:
        ...
        explicit resources_hash_node(
          const std::string &key, const resources &value) :
    ...
    class resources_hash_table {
      public:
        ...
        explicit resources_hash_table(unsigned int size) :
          m_nodes(size_t(size > 2 ? size : 2), nullptr) {}
        ...
        resources_hash_node *find(const std::string &key) {
          ...
          while (m_nodes [index] != nullptr) {
            ...
            if (
              hash == node->hash
              && std::strcmp(key.c_str(), node->key.c_str()) == 0) {
      ...
      private:
        void insert(resources_hash_node *node) {
          ...
          if (m_size >= m_nodes.size() / 2) { rehash(m_nodes.size() * 2); }
        ...
        index_type insert_at_next_ready(resources_hash_node *node) {
          while (m_nodes [m_next_ready] != nullptr)
    ...
    class code_block {
      public:
        ...
        code_block() :
          ...
          m_buffer(new char [BUFFER_SIZE], std::default_delete<char []>()) {}

        static std::ostream &generate_header(
          std::ostream &out, const std::string &mark = "") {
          write_lines(out, code_header1) << std::endl;
          if (! mark.empty()) {
            auto var = to_var(mark);
            out << "#define orz_resources_get __orz_" << var << "_get"
                << std::endl;
            out << std::endl;
          }
        ...
        std::ostream &declare_data(
          std::ostream      &out,
          std::istream      &mem,
          const std::string &id,
          const std::string &indent    = "",
          size_t            *data_size = nullptr) {
          out << indent << "static const uint64_t orz_resources_table_item_"
              << id << "[] = {" << std::endl;
          ...
          out << indent << "};" << std::endl;
          out << indent << "/* static const size_t orz_resources_table_item_"
              << id << "_size = " << memory_size << "UL; */" << std::endl;

          std::string table_item_size_name = std::string(
                                               "orz_resources_table_item_")
                                           + id + "_size";
          m_table_item_size.insert(
            std::make_pair(table_item_size_name, memory_size));
        ...
        std::ostream &declare_node(
          std::ostream                  &out,
          const std::string             &id,
          const std::string             &key,
          resources_hash_node::hash_type hash,
          int64_t                        next,
          const std::string             &indent = "") {
          std::string table_item_size_name = std::string(
                                               "orz_resources_table_item_")
          ...
          out << std::dec << indent << "{ \"" << key << "\", " << hash << ", "
              << next << "," << std::endl;
          out << indent << "  (const char *)orz_resources_table_item_" << id
              << ", " << memory_size << "UL }";
          return out;
        }

        std::ostream &declare_empty_node(
    ...
    static inline std::string get_filename(const std::string &path) {
      ...
      else if (unix_sep_pos != std::string::npos && unix_sep_pos > sep_pos)
        sep_pos = unix_sep_pos;
    ...
    bool compiler::compile(
      const std::vector<orz::resources::resources> &in_resources,
      ...
      for (auto &resources: in_resources) {
        ...
        if (try_found) {
          ...
          oss << "[Error] line(" << resources.line << "): "
              << "Conflict URL with line(" << conflict_resources.line << ") \""
              << resources.url << "\"";
      ...
      for (size_t i = 0; i < nodes.size(); ++i) {
        ...
        if (! file->is_open()) {
          std::ostringstream oss;
          oss << "[Error] line(" << res.line << "): "
              << "Can not access file \"" << res.path << "\"";
          m_last_error_message = oss.str();
          return false;
        }
      }
      ...
      for (size_t i = 0; i < nodes.size(); ++i) {
        ...
        std::cout << "[Info] "
                  << "Compiling \"" << node->value.url << "\"." << std::flush;
        size_t data_size = 0;

        auto &file = *in_files [i];
        coder.declare_data(out_source, file, std::to_string(i), "", &data_size)
          << std::endl;
      ...
      write_lines(out_source, code_source_declare_orz_resources_node)
        << std::endl;
      ...
      write_lines(out_source, code_source_declare_orz_resources_table_tail)
        << std::endl;
      write_lines(out_source, code_source_declare_orz_resources_table_size)
        << std::endl;
      write_lines(out_source, code_source_declare_orz_resources_table_find)
        << std::endl;
      write_lines(out_source, code_source_declare_orz_resources_get)
        << std::endl;
    ...
    bool compiler::compile(
      ...
      std::cout << "[Info] "
                << "Found " << list.size() << " files." << std::endl;
    ...
    bool compiler::compile(
      ...
      if (! out_source.is_open()) {
        std::ostringstream oss;
        oss << "[Error] "
            << "Can not open output file \"" << source_filename << "\".";
        m_last_error_message = oss.str();
        return false;
      }
      ...
      if (! up2date_header(header_filename, memory_out_header_content)) {
        ...
        if (! out_header.is_open()) {
          std::ostringstream oss;
          oss << "[Error] "
              << "Can not open output file \"" << header_filename << "\".";
          m_last_error_message = oss.str();
          return false;
        }
    ...
    bool compiler::compile(
      ...
      if (orz::isfile(path)) {
        ...
        if (! in_source.is_open()) {
          std::ostringstream oss;
          oss << "[Error] "
              << "Can not access input file \"" << path << "\"";
          m_last_error_message = oss.str();
          return false;
        }
        std::cout << "[Info] "
                  << "Open file \"" << path << "\"." << std::endl;
        return compile(in_source, header_filename, source_filename);
      } else if (orz::isdir(path)) {
        m_input_directory = m_working_directory;  // make sure input directory
                                                  // in working path change
        auto filenames    = orz::FindFilesRecursively(path);
        std::cout << "[Info] "
                  << "Found " << filenames.size() << " files in folder \""
                  << path << "\"." << std::endl;
        ...
        std::ostringstream oss;
        oss << "[Error] "
            << "Can not access input path \"" << path
            << "\", is a file or dir?";
        m_last_error_message = oss.str();
        return false;
      }
    ...
    bool compiler::up2date_header(
      const std::string &header_filename, const std::string &content) {
      std::ifstream header_file(header_filename);
      if (! header_file.is_open()) return false;
      std::ostringstream ready_out_header;
      ready_out_header << header_file.rdbuf();
    ...
    compiler::compiler() { m_working_directory = orz::getcwd(); }
  }  // namespace resources
}  // namespace orz
