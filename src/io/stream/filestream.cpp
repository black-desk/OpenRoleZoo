...
namespace orz
{
  ...
  FileInputStream::FileInputStream(const std::string &file, bool istxt) :
    ...
    if (nfind >= 0) {
      ...
      if (nfind >= 0) { m_workpath = m_file.substr(0, nfind + 1); }
    }
  }

  FileInputStream::~FileInputStream() {}

  const char *FileInputStream::getworkpath() { return m_workpath.c_str(); }
  ...
  const char *FileOutputStream::getworkpath() { return m_workpath.c_str(); }
