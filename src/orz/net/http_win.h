...
namespace orz
{
  ...
  static DWORD default_http_flags() {
    return  // flags common to open functions
      ...
      | INTERNET_FLAG_READ_PREFETCH | INTERNET_FLAG_NO_COOKIES
      | INTERNET_FLAG_NO_AUTH | INTERNET_FLAG_RESTRICTED_ZONE
  ...
  std::string http_request_core(
    ...
    HINTERNET hInternet = InternetOpenA(
      ...
      0);
    if (hInternet == NULL) {
      ORZ_LOG(INFO) << "Internet open error: " << std::hex
    ...
    HINTERNET hConnect = InternetConnectA(
      ...
      0);
    if (hConnect == NULL) {
      ORZ_LOG(INFO) << "Internet connect error: " << std::hex
                    << format_message(GetLastError());
      return report;
    }
    ...
    // Request
    HINTERNET hOpenRequest = HttpOpenRequestA(
      hConnect,
      verb_string(verb),
      url.target().c_str(),
      NULL,
      NULL,
      (LPCSTR *)("*/*"),
      defulat_protocol_flags(url.protocol()),
      0);
    if (hOpenRequest == NULL) {
      ORZ_LOG(INFO) << "Http open request error: " << std::hex
                    << format_message(GetLastError());
      return report;
    }
    ...
    // charset=utf-8"; std::string header = "Content-Type:application/json;
    // charset=utf-8";

    // Send
    BOOL bRequest = HttpSendRequestA(
      hOpenRequest,
    ...
    if (! bRequest) {
      ORZ_LOG(INFO) << "Http send request error: " << std::hex
                    << format_message(GetLastError());
      return report;
    }
    ...
    DWORD  dwByteRead      = 0;
    while (
      InternetReadFile(hOpenRequest, szBuffer, sizeof(szBuffer), &dwByteRead)
      && dwByteRead > 0) {
