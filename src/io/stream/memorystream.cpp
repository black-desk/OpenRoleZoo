...
namespace orz
{
  ...
  MemoryInputStream::MemoryInputStream(const char *buf, int64_t len) {
    m_buf = std::shared_ptr<char>(
      new char [len], std::default_delete<char []>());
  ...
  void MemoryOutputStream::grow(int64_t mincapacity) {
    ...
    if (ncur - mincapacity < 0) { ncur = mincapacity; }
  ...
  int64_t MemoryOutputStream::write(const char *buf, int64_t len) {
    if (m_buf != nullptr) {
      if (m_pos > std::numeric_limits<int64_t>::max() - len) { return -1; }
      if (m_pos + len > m_len) { grow(m_pos + len); }
