...
namespace orz
{
  ...
  jug::jug() : jug(Piece::NIL) {}

  jug::jug(Piece::Type type) : m_pie(Piece::Get(type)) {}

  jug::jug(std::nullptr_t _) : m_pie(std::make_shared<NilPiece>()) {}

  jug::jug(int val) : m_pie(std::make_shared<IntPiece>(val)) {}

  jug::jug(float val) : m_pie(std::make_shared<FloatPiece>(val)) {}

  jug::jug(const std::string &val) :
    m_pie(std::make_shared<StringPiece>(val)) {}

  jug::jug(bool val) : m_pie(std::make_shared<BooleanPiece>(val)) {}

  jug::jug(const binary &val) : m_pie(std::make_shared<BinaryPiece>(val)) {}

  bool jug::valid(Piece::Type type) const {
    return m_pie && m_pie->type() == type;
  }

  bool jug::valid() const { return ! valid(Piece::NIL); }

  jug &jug::operator=(std::nullptr_t _) {
    switch (m_pie->type()) {
      case Piece::NIL: break;
      default: m_pie = std::make_shared<NilPiece>(); break;
    }
    return *this;
  }
  ...
  jug &jug::operator=(int val) {
    switch (m_pie->type()) {
      case Piece::INT:
        ...
        break;
      default: m_pie = std::make_shared<IntPiece>(val); break;
    }
    return *this;
  }
  ...
  jug &jug::operator=(float val) {
    switch (m_pie->type()) {
      case Piece::FLOAT:
        ...
        break;
      default: m_pie = std::make_shared<FloatPiece>(val); break;
    }
    return *this;
  }
  ...
  jug &jug::operator=(const std::string &val) {
    switch (m_pie->type()) {
      case Piece::STRING:
        ...
        break;
      default: m_pie = std::make_shared<StringPiece>(val); break;
    }
    return *this;
  }
  ...
  jug &jug::operator=(const binary &val) {
    switch (m_pie->type()) {
      case Piece::BINARY:
        ...
        break;
      default: m_pie = std::make_shared<BinaryPiece>(val); break;
    }
    return *this;
  }
  ...
  jug &jug::operator=(bool val) {
    switch (m_pie->type()) {
      case Piece::BOOLEAN:
        ...
        break;
      default: m_pie = std::make_shared<BooleanPiece>(val); break;
    }
    return *this;
  }
  ...
  int jug::to_int() const {
    switch (m_pie->type()) {
      case Piece::INT: return reinterpret_cast<IntPiece *>(m_pie.get())->get();
      case Piece::FLOAT:
      ...
      default: throw Exception("Can not convert this jug to int");
    }
  }
    ...
    switch (m_pie->type()) {
      ...
      case Piece::STRING:
        return static_cast<float>(std::atof(
          reinterpret_cast<StringPiece *>(m_pie.get())->get().c_str()));
      default: throw Exception("Can not convert this jug to float");
    }
  }
  ...
  std::string jug::to_string() const {
    switch (m_pie->type()) {
      ...
      default: throw Exception("Can not convert this jug to string");
    }
  }
  ...
  binary jug::to_binary() const {
    switch (m_pie->type()) {
      ...
      default: throw Exception("Can not convert this jug to binary");
    }
  }
  ...
  size_t jug::size() const {
    switch (m_pie->type()) {
      ...
      default: throw Exception("This jug has no method size()");
    }
  }
  ...
  jug jug::index(size_t i) {
    switch (m_pie->type()) {
      case Piece::NIL: m_pie = Piece::Get(Piece::LIST);
      case Piece::LIST: {
      ...
      default: throw Exception("This jug has no method index(i)");
    }
  }
  ...
  jug jug::index(size_t i, const jug &value) {
    switch (m_pie->type()) {
      ...
      default: throw Exception("This jug has no method index(i, value)");
    }
  }
  ...
  jug jug::index(const std::string &key) {
    switch (m_pie->type()) {
      case Piece::NIL: m_pie = Piece::Get(Piece::DICT);
      case Piece::DICT: {
      ...
      default: throw Exception("This jug has no method index(key)");
    }
  }
  ...
  jug jug::index(const std::string &key, const jug &value) {
    switch (m_pie->type()) {
      ...
      default: throw Exception("This jug has no method index(key, value)");
    }
  }
  ...
  std::vector<std::string> jug::keys() const {
    switch (m_pie->type()) {
      ...
      default: throw Exception("This jug has no method keys()");
    }
  }
  ...
  jug &jug::set_bits(const void *buffer, size_t size) {
    switch (m_pie->type()) {
      case Piece::NIL: m_pie = Piece::Get(Piece::BINARY);
      case Piece::BINARY: {
        auto buff = reinterpret_cast<BinaryPiece *>(m_pie.get());
        buff->set_bits(buffer, size);
        return *this;
      }
      default: throw Exception("This jug has no method set_bits(buffer, size)");
    }
  }
  ...
  Piece *jug::raw() { return m_pie.get(); }

  const Piece *jug::raw() const { return m_pie.get(); }

  const std::string jug::str() const { return m_pie->str(); }

  const std::string jug::repr() const { return m_pie->repr(); }
  ...
  jug jug_read(std::istream &in) { return Piece::Read(in); }
  ...
  jug::operator bool() const { return to_bool(); }

  jug::operator int() const { return to_int(); }

  jug::operator float() const { return to_float(); }

  jug::operator std::string() const { return to_string(); }

  jug::operator binary() const { return to_binary(); }

  std::string to_string(const jug &obj) { return obj.str(); }
