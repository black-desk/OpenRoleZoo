...
namespace orz
{
  ...
  static bool parse_null(json_iterator &beg, jug &value) {
    beg = jump_space(beg);
    if (beg == beg.end())
      ORZ_LOG(ERROR) << "syntax error: converting empty json to null" << crash;
  ...
  static jug parse_boolean(json_iterator &beg) {
    beg = jump_space(beg);
    if (beg == beg.end())
      ORZ_LOG(ERROR) << "syntax error: converting empty json to boolean"
                     << crash;
    jug result;
  ...
  static std::string parse_string(json_iterator &beg) {
    beg = jump_space(beg);
    if (beg == beg.end())
      ORZ_LOG(ERROR) << "syntax error: converting empty json to string"
                     << crash;
    if (*beg != '"')
      ORZ_LOG(ERROR) << "syntax error: string begin with " << *beg << crash;
    std::string value;
    auto        it            = beg;
    bool        slant         = false;
    int         unicode_index = 0;
    ...
    while (++it != it.end()) {
      if (unicode_index > 0) {
        int ch = char2hex(*it);
        if (ch < 0)
          ORZ_LOG(ERROR) << "syntax error: unrecognized unicode" << crash;
        ...
        switch (*it) {
          ...
          case 'u': unicode_index = 1; break;
          default: value.push_back(*it); break;
        }
        slant = false;
        continue;
      } else if (*it == '\\') {
  ...
  static jug parse_number(json_iterator &beg) {
    beg = jump_space(beg);
    if (beg == beg.end())
      ORZ_LOG(ERROR) << "syntax error: converting empty json to number"
                     << crash;
    jug         result;
  ...
  static jug parse_list(json_iterator &beg) {
    beg = jump_space(beg);
    if (beg == beg.end())
      ORZ_LOG(ERROR) << "syntax error: converting empty json to list" << crash;
    if (*beg != '[')
      ORZ_LOG(ERROR) << "syntax error: list begin with " << *beg << crash;
  ...
  static jug parse_dict(json_iterator &beg) {
    beg = jump_space(beg);
    if (beg == beg.end())
      ORZ_LOG(ERROR) << "syntax error: converting empty json to dict" << crash;
    if (*beg != '{')
      ORZ_LOG(ERROR) << "syntax error: dict begin with " << *beg << crash;
    ...
    while (++it != it.end()) {
      ...
      it                    = jump_space(it);
      if (it == it.end() || *it != ':')
        ORZ_LOG(ERROR) << "syntax error: dict key:value must split with :"
                       << crash;
      ++it;
      jug local_value = parse_value(it);
  ...
  using command_handler =
    std::function<jug(const std::vector<std::string> &args)>;
  ...
  static command_handler registered_command(const std::string &command) {
    static std::unordered_map<std::string, command_handler> command_map = {
      { "date", pack_date },         { "time", pack_time },
      { "datetime", pack_datetime }, { "nil", pack_nil },
      { "binary", pack_error },      { "file", pack_file },
      { "base64", pack_base64 },
    };
  ...
  static jug parse_value(json_iterator &beg) {
    beg = jump_space(beg);
    if (beg == beg.end())
      ORZ_LOG(ERROR) << "syntax error: converting empty json" << crash;
    jug  value;
    auto it = beg;
  ...
  jug json2jug(const std::string &json) {
    ...
    } catch (const Exception &) { return orz::jug(); }
  }

  std::string jug2json(const orz::jug &obj) { return obj.repr(); }
  ...
  std::string form_encode(const orz::jug &obj) {
    if (! obj.valid(Piece::DICT))
      ORZ_LOG(ERROR) << "form encoding only supporting dict" << crash;
    std::ostringstream oss;
    int                first = true;
