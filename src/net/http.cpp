...
namespace orz
{
  URL::URL(const std::string &url) : m_url(url), m_valid(false) {
    auto anchor = m_url.find(':', 0);
    if (anchor != std::string::npos) {
      if (
        anchor + 2 < m_url.length() && m_url [anchor + 1] == '/'
        && m_url [anchor + 2] == '/') {
    ...
    if (port_anchor <= target_anchor) {
      ...
      if (target_anchor == std::string::npos)
        port_string = m_url.substr(port_anchor + 1);
      else
        port_string = m_url.substr(
          port_anchor + 1, target_anchor - port_anchor);
  ...
  std::string http_request(
    const URL &url, http::VERB verb, const std::string &data) {
