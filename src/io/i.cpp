...
namespace orz
{
  ...
  int imemorystream::imemorybuffer::overflow(int c) { return c; }

  std::streambuf *imemorystream::imemorybuffer::setbuf(
  ...
  int imemorystream::imemorybuffer::underflow() { return EOF; }

  int imemorystream::imemorybuffer::uflow() { return EOF; }

  std::streampos imemorystream::imemorybuffer::seekoff(
    std::streamoff          off,
    ...
    auto pos = gptr();
    switch (way) {
      case ios_base::beg: pos = eback() + off; break;
      case ios_base::cur: pos = gptr() + off; break;
      case ios_base::end: pos = egptr() + off; break;
      default: pos = gptr(); break;
    }
    setg(eback(), pos, egptr());
    return gptr() - eback();
  }

  std::streampos imemorystream::imemorybuffer::seekpos(
  ...
  MemoryFILE::MemoryFILE(::FILE *file) : m_file(file) {}
  ...
  size_t fwrite(
    const void *ptr, size_t size, size_t count, MemoryFILE *stream) {
  ...
  int fseek(MemoryFILE *stream, long int offset, int origin) {
    ...
    switch (origin) {
      case SEEK_SET: new_index = static_cast<size_t>(offset); break;
      case SEEK_CUR: new_index = new_index + offset; break;
      case SEEK_END: new_index = stream->m_size + offset; break;
      default: break;
    }
  ...
  void rewind(MemoryFILE *stream) { fseek(stream, 0, SEEK_SET); }
