...
namespace orz
{
  ...
  static inline void _Concat_str(std::ostream &out) {
    (decltype(_Concat_str(out))());
  }

  template <typename T, typename... Args>
  static inline void _Concat_str(std::ostream &out, T &&t, Args &&...args) {
    _Concat_str(out << std::forward<T>(t), std::forward<Args>(args)...);
  }
  ...
  std::vector<std::string> Split(
    const std::string &str, char ch = ' ', size_t _size = 0);

  std::vector<std::string> Split(
    const std::string &str, const std::string sep = " ", size_t _size = 0);

  std::string Join(
    const std::vector<std::string> &list, const std::string &sep);
  ...
  std::string to_string(
    time_point tp, const std::string &format = "%Y-%m-%d %H:%M:%S");
