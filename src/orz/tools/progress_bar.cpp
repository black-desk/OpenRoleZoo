...
namespace orz
{
  ...
  static bool append_string(
    std::string &base, const std::string &add, size_t limit) {
    if (base.length() + add.length() > limit) return false;
    base.insert(base.end(), add.begin(), add.end());
    return true;
  }

  static void output_string(std::ostream &out) {
    (decltype(output_string(out))());
  }
  ...
  inline const std::string concat_string(
    std::ostringstream &oss, Args &&...args) {
    oss.str("");
  ...
  std::string to_string(microseconds us, size_t limit) {
    ...
    if (day && ! append_string(format, concat_string(oss, day, 'd'), limit))
      return format;
    if (hour && ! append_string(format, concat_string(oss, hour, 'h'), limit))
      return format;
    if (
      minute && ! append_string(format, concat_string(oss, minute, 'm'), limit))
      return format;
    if (
      second && ! append_string(format, concat_string(oss, second, 's'), limit))
      return format;
    if (
      ! day && ! hour && ! minute && ms
      && ! append_string(format, concat_string(oss, ms, "ms"), limit))
      return format;

    return format;
  }

  int progress_bar::next() { return next(m_step); }
  ...
  microseconds progress_bar::used_time() const {
    switch (m_stat) {
      default: return microseconds(0);
      case WAITING: return microseconds(0);
      case RUNNING:
        return duration_cast<microseconds>(
                 system_clock::now() - m_start_time_point)
             - m_paused_duration;
      case PAUSED:
        return duration_cast<microseconds>(
  ...
  void progress_bar::start() {
    switch (m_stat) {
      ...
      case RUNNING: break;
      case PAUSED:
        m_paused_duration += duration_cast<microseconds>(
          system_clock::now() - m_pause_time_point);
        reset();
        break;
      case STOPPED:
        m_start_time_point = system_clock::now();
  ...
  void progress_bar::stop() {
    switch (m_stat) {
      default: m_stop_time_point = system_clock::now(); break;
      case WAITING:
        m_start_time_point = system_clock::now();
        m_stop_time_point  = m_start_time_point;
        break;
      case RUNNING: m_stop_time_point = system_clock::now(); break;
      case PAUSED:
        m_paused_duration += duration_cast<microseconds>(
          system_clock::now() - m_pause_time_point);
  ...
  void progress_bar::pause() {
    switch (m_stat) {
      default: m_pause_time_point = system_clock::now(); break;
      case WAITING:
        m_start_time_point = system_clock::now();
        m_pause_time_point = m_start_time_point;
        break;
      case RUNNING: m_pause_time_point = system_clock::now(); break;
      case PAUSED: break;
      case STOPPED: break;
    }
    m_stat = PAUSED;
  ...
  void progress_bar::autostop(bool flag) { m_autostop = flag; }

  bool progress_bar::autostop() const { return m_autostop; }

  int progress_bar::value() const { return m_value; }

  int progress_bar::max() const { return m_max; }

  int progress_bar::min() const { return m_min; }

  void progress_bar::set_value(int value) { m_value = value; }

  void progress_bar::set_min(int min) { m_min = min; }

  void progress_bar::set_max(int max) { m_max = max; }

  progress_bar::status progress_bar::stat() const { return m_stat; }

  static const char running_status []  = { '-', '\\', '|', '/' };
  static const auto running_status_num = sizeof(running_status)
                                       / sizeof(running_status [0]);

  std::ostream &progress_bar::show(std::ostream &out) const {
    std::ostringstream oss;

    switch (m_stat) {
      case WAITING: oss << "[~]"; break;
      case RUNNING:
        oss << '[' << running_status [m_show_count % running_status_num] << ']';
        m_show_count++;
        m_show_count %= running_status_num;
        break;
      case PAUSED: oss << "[=]"; break;
      case STOPPED: oss << "[*]"; break;
    }
  ...
  void progress_bar::sample() {
    ...
    auto sample_time_duration = duration_cast<microseconds>(
  ...
  std::ostream &progress_bar::wait_show(int ms, std::ostream &out) const {
    auto now_time_point = system_clock::now();
    auto wait_duration  = duration_cast<milliseconds>(
