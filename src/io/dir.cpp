...
namespace orz
{
  ...
  bool mkdir(const std::string &dir) {
    ...
    for (size_t i = 1; i <= path.size(); ++i) {
      ...
      auto local_path = orz::Join(
        std::vector<std::string>(path.begin(), path.begin() + i),
        FileSeparator());
  ...
  bool copy(
    const std::string &fromfile, const std::string &tofile, bool force) {
#if ORZ_PLATFORM_OS_WINDOWS
    return CopyFileA(fromfile.c_str(), tofile.c_str(), ! force) != FALSE;
#elif ORZ_PLATFORM_OS_LINUX
    return std::system(
             orz::Concat(force ? "cp -f " : "cp ", fromfile, ' ', tofile)
               .c_str())
        == 0;
#else
  ...
  bool cd(const std::string &path) { return CHDIR(path.c_str()) == 0; }
