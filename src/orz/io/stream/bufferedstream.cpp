...
namespace orz
{
  ...
  BufferedInputStream::BufferedInputStream(
    std::shared_ptr<InputStream> in, int64_t size) :
    FilterInputStream(in) {
    m_len     = size;
    m_pos     = 0;
    m_datalen = 0;
    m_buf     = std::shared_ptr<char>(
      new char [m_len], std::default_delete<char []>());
  }

  /////////////////////////////////////
  int64_t BufferedOutputStream::write(const char *buf, int64_t len) {
    if (m_out == nullptr) { return -1; }
    ...
    if (m_len - m_pos > 0) { memcpy(m_buf.get() + m_pos, buf, m_len - m_pos); }
  ...
  BufferedOutputStream::BufferedOutputStream(
    std::shared_ptr<OutputStream> out, int64_t size) :
