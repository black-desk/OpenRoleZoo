...
namespace orz
{
  ...
  static std::vector<std::string> FindFilesCore(
    const std::string &path, std::vector<std::string> *dirs = nullptr) {
  ...
  std::vector<std::string> FindFilesRecursively(
    ...
    while (! work.empty()) {
      ...
      for (auto &file: files)
        result.push_back(local_path + FileSeparator() + file);
      for (auto &dir: dirs)
        work.push({ local_path + FileSeparator() + dir, local_depth + 1 });
    }
    return result;
  }
  ...
  std::vector<std::string> FindFlodersRecursively(
    ...
    while (! work.empty()) {
      ...
      files = FindFiles(path + FileSeparator() + local_path, dirs);
      for (auto &dir: dirs)
        result.push_back(local_path + FileSeparator() + dir);
      for (auto &dir: dirs)
        work.push({ local_path + FileSeparator() + dir, local_depth + 1 });
    }
    return result;
