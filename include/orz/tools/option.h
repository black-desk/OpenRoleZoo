  ...
  namespace arg
  {
    ...
    static inline std::shared_ptr<Value> make_shared(ValueType type) {
      switch (type) {
        ...
        case STRING: return std::make_shared<ValueDefinition<STRING>>();
        case FLOAT: return std::make_shared<ValueDefinition<FLOAT>>();
        case INT: return std::make_shared<ValueDefinition<INT>>();
        case BOOLEAN: return std::make_shared<ValueDefinition<BOOLEAN>>();
      }
    }
    ...
    class ValueCommon {
      public:
        ...
        ValueCommon(ValueType type) : m_value(make_shared(type)) {}

        bool valid() const { return m_value != nullptr; }

        bool valid(ValueType type) {
          return m_value != nullptr && m_value->type() == type;
        }
        ...
        self &reset(ValueType type) {
          ...
          switch (type) {
            default: break;
            case STRING: m_value.reset(new ValueDefinition<STRING>); break;
            case FLOAT: m_value.reset(new ValueDefinition<FLOAT>); break;
            case INT: m_value.reset(new ValueDefinition<INT>); break;
            case BOOLEAN: m_value.reset(new ValueDefinition<BOOLEAN>); break;
          }
          return *this;
        }
        ...
        bool set(const Type<STRING>::declare &value) {
          ...
          switch (m_value->type()) {
            ...
            case STRING: {
              auto self_value = dynamic_cast<ValueDefinition<STRING> *>(
                m_value.get());
              self_value->set(value);
              return true;
            }
            case FLOAT: {
              auto self_value = dynamic_cast<ValueDefinition<FLOAT> *>(
                m_value.get());
              self_value->set(Type<FLOAT>::declare(std::atof(value.c_str())));
              return true;
            }
            case INT: {
              auto self_value = dynamic_cast<ValueDefinition<INT> *>(
                m_value.get());
              self_value->set(Type<INT>::declare(std::atol(value.c_str())));
              return true;
            }
            case BOOLEAN: {
              auto self_value = dynamic_cast<ValueDefinition<BOOLEAN> *>(
                m_value.get());
        ...
        bool set(const char *value) {
          return set(Type<STRING>::declare(value));
        }
        ...
        bool set(unsigned short value) {
          return set(Type<INT>::declare(value));
        }
        ...
        Type<STRING>::declare to_string() const {
          if (m_value == nullptr) { return Type<STRING>::default_value; }
          switch (m_value->type()) {
            default: return Type<STRING>::default_value;
            case STRING: {
              auto self_value = dynamic_cast<ValueDefinition<STRING> *>(
                m_value.get());
        ...
        Type<FLOAT>::declare to_float() const {
          if (m_value == nullptr) { return Type<FLOAT>::default_value; }
          switch (m_value->type()) {
            default: return Type<FLOAT>::default_value;
            case STRING: {
              auto self_value = dynamic_cast<ValueDefinition<STRING> *>(
                m_value.get());
        ...
        Type<INT>::declare to_int() const {
          if (m_value == nullptr) { return Type<INT>::default_value; }
          switch (m_value->type()) {
            default: return Type<INT>::default_value;
            case STRING: {
              auto self_value = dynamic_cast<ValueDefinition<STRING> *>(
                m_value.get());
        ...
        Type<BOOLEAN>::declare to_boolean() const {
          if (m_value == nullptr) { return Type<BOOLEAN>::default_value; }
          switch (m_value->type()) {
            default: return Type<BOOLEAN>::default_value;
            case STRING: {
              auto self_value = dynamic_cast<ValueDefinition<STRING> *>(
                m_value.get());
        ...
        operator ValueDefinition<_TYPE>() {
          if (this->valid(_TYPE))
            return *dynamic_cast<ValueDefinition<_TYPE> *>(this->m_value.get());
    ...
    inline std::ostream &operator<<(
      std::ostream &out, const ValueCommon &object) {
      switch (object.type()) {
        default: return out;
        case STRING: return out << object.to_string();
        case FLOAT: return out << object.to_float();
        case INT: return out << object.to_int();
        case BOOLEAN: return out << (object.to_boolean() ? "true" : "false");
      }
      return out;
    }
    ...
    class Option {
      public:
        ...
        self *name(const std::initializer_list<std::string> &_string_list) {
          return this->name(
            std::set<std::string>(_string_list.begin(), _string_list.end()));
        }
        ...
        bool parse(const std::string &name, const std::string &value) {
          ...
          if (this->type() != BOOLEAN && value.empty()) { return false; }
    ...
    inline std::ostream &operator<<(std::ostream &out, const Option &option) {
      ...
      out << "]";
      if (option.property() == REQUIRED) { out << " [REQUIRED]"; }
    ...
    static inline int edit_distance(
...
#undef __EDIT_DIST
    ...
    class OptionSet {
      public:
        ...
        Option *add(
          ValueType type, const std::initializer_list<std::string> &list) {
        ...
        class option_iterator {
          public:
            ...
            using inner_iterator =
              std::vector<std::shared_ptr<Option>>::iterator;
            ...
            const self operator++(int) { return self(this->m_iter++); }
            ...
            bool operator==(const self &rhs) { return m_iter == rhs.m_iter; }

            bool operator!=(const self &rhs) { return m_iter != rhs.m_iter; }

            Option &operator*() { return **m_iter; }

            Option *operator->() { return (*m_iter).get(); }
          private:
        ...
        class const_option_iterator {
          public:
            ...
            explicit const_option_iterator(inner_iterator iter) :
              m_iter(iter) {}
            ...
            const Option &operator*() { return **m_iter; }

            const Option *operator->() { return (*m_iter).get(); }
          private:
            inner_iterator m_iter;
        };

        option_iterator begin() {
          return option_iterator(m_option_list.begin());
        }

        option_iterator end() { return option_iterator(m_option_list.end()); }

        const_option_iterator begin() const {
          return const_option_iterator(m_option_list.cbegin());
        }

        const_option_iterator end() const {
          return const_option_iterator(m_option_list.cend());
        }

        const_option_iterator cbegin() const {
          return const_option_iterator(m_option_list.cbegin());
        }

        const_option_iterator cend() const {
          return const_option_iterator(m_option_list.cend());
        }
      private:
