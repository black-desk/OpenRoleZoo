...
namespace orz
{
  class rsa_key {
    ...
    public:
      ...
      key_type type() const { return m_type; }

      inner_type *inner() { return m_rsa; }

      const inner_type *inner() const { return m_rsa; }
...
#  endif
        if (key_file == nullptr) return nullptr;
        need    close_file(fclose, key_file);
        rsa_st *rsa = PEM_read_RSAPrivateKey(
          key_file, nullptr, nullptr, nullptr);
        if (rsa == nullptr) return nullptr;
  ...
  void free_rsa_key(const rsa_key *key) { rsa_key::free(key); }

  static std::string block_transfer(
  ...
  std::string rsa_mem_private_encode(
    const std::string &buffer, const std::string &data) {
  ...
  std::string rsa_mem_public_decode(
    const std::string &buffer, const std::string &data) {
  ...
  std::string rsa_mem_public_encode(
    const std::string &buffer, const std::string &data) {
    auto key = load_mem_public_rsa_key(buffer);
    if (key == nullptr) return std::string();
    need free_key(free_rsa_key, key);
  ...
  std::string rsa_mem_private_decode(
    const std::string &buffer, const std::string &data) {
    auto key = load_mem_private_rsa_key(buffer);
    if (key == nullptr) return std::string();
    need free_key(free_rsa_key, key);
  ...
  std::string rsa_private_encode(rsa_key *key, const std::string &data) {
    ...
    try {
      return block_transfer(
        data,
        size_t(block_size),
        std::bind(rsa_encode_block, key, std::placeholders::_1));
  ...
  std::string rsa_public_decode(rsa_key *key, const std::string &data) {
    ...
    try {
      return block_transfer(
        data,
        size_t(block_size),
        std::bind(rsa_decode_block, key, std::placeholders::_1));
    } catch (const Exception &) { return std::string(); }
  }
    ...
    switch (key->type()) {
      case rsa_key::PUBLIC:
        rsize = RSA_public_encrypt(
          ...
          rsa,
          RSA_PKCS1_PADDING /* RSA_PKCS1_PADDING RSA_NO_PADDING */);
        break;
