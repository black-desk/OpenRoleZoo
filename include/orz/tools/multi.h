...
namespace orz
{
  ...
  class Multi {
    public:
      ...
      explicit Multi(size_t N, Args &&...args) : m_cores(N) {
        for (auto &core: this->m_cores)
          core = std::make_shared<T>(std::forward<Args>(args)...);
      }
       ...
       * @tparam FUNC function type, should be Ret(T*, Args...) or
       * Ret(T::*)(Args...)
      ...
      void each(FUNC func, Args &&...args) {
        for (auto &core: this->m_cores)
          std::bind(func, &*core, std::forward<Args>(args)...)();
      }
    private:
