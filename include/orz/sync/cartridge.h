...
namespace orz
{
  ...
  class Cartridge {
    public:
      ...
      /**
       * @brief fire Asynchronous build and fire bullet, first calls the bullet,
      ...
      void fire(
        ...
        const shell_type  &shell = nullptr);
    ...
    private:
      ...
      std::condition_variable
                        fire_cond;  ///< condition to tell if fire finished
      std::atomic<bool> dry;        ///< object only work when dry is true

      int
        signet;  ///< the argument to call `bullet(signet)` and `shell(signet)`
