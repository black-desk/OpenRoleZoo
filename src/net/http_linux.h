...
#  else  // !ORZ_WITH_OPENSSL
        ...
        auto subject = X509_NAME_oneline(
...
#  endif  // ORZ_WITH_OPENSSL
  ...
  static struct hostent *gethostbyname_local(
    const char *name, struct hostent &result_buf) {
...
#  else
    ...
    int err;
    if (
      gethostbyname_r(name, &result_buf, buf, buflen, &result, &err) != 0
      || result == nullptr) {
      return nullptr;
    }
#  endif
  ...
  std::string http_request_core(
    ...
    if (host == nullptr) { ORZ_LOG(ERROR) << "gethostbyname error" << crash; }
