...
namespace orz
{
  ...
  template <typename T>
  ORZ_LITE_THREAD_LOCAL typename __thread_local_lite_context<T>::context
    __thread_local_lite_context<T>::m_ctx = nullptr;

  template <typename T>
  typename __thread_local_lite_context<T>::context
  __thread_local_lite_context<T>::swap(
    typename __thread_local_lite_context<T>::context ctx) {
    auto pre_ctx = m_ctx;
    m_ctx        = ctx;
    return pre_ctx;
  }
  ...
  template <typename T>
  void __thread_local_lite_context<T>::set(
    typename __thread_local_lite_context<T>::context ctx) {
    m_ctx = ctx;
  }

  template <typename T>
  typename __thread_local_lite_context<T>::context const
  __thread_local_lite_context<T>::get() {
  ...
  template <typename T>
  typename __thread_local_lite_context<T>::context const
  __thread_local_lite_context<T>::try_get() {
    return m_ctx;
  }
