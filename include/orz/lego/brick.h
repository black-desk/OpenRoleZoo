...
namespace orz
{
  ...
  class function_brick : public brick<I, O> {
    public:
      ...
      virtual O work(const I &input) override { return this->m_step(input); }
    private:
      step_type m_step;
  };
  ...
  class wide_brick : public brick<IOList<I>, IOList<O>> {
    public:
      wide_brick(typename brick<I, O>::ptr core) : m_gun(0) {
      ...
      wide_brick(const std::vector<typename brick<I, O>::ptr> &cores) :
        ...
        if (m_core.empty())
          orz::Log(FATAL) << "Can not init with empty cores" << crash;
      }
  ...
  typename brick<typename BRICK::IType, typename BRICK::OType>::ptr make_brick(
    Args &&...args) {
    return std::make_shared<BRICK>(std::forward<Args>(args)...);
  };

  template <typename BRICK, typename... Args>
  typename brick<IOList<typename BRICK::IType>, IOList<typename BRICK::OType>>::
    ptr
    make_wide_brick(const size_t N, Args &&...args) {
    ...
    if (N == 1) {
      ...
      for (auto &core: cores)
        core = make_brick<BRICK>(std::forward<Args>(args)...);
