...
namespace orz
{
  ...
  ModelFileInputStream::ModelFileInputStream(const std::string &file) {
...

    {
      ...
      if (! reader.is_open()) {
        orz::Log(orz::ERROR)
          << "open the model file:" << file << " failed!" << orz::crash;
      }
      reader.read((char *)&mask, 4);
      if (reader.bad()) {
        orz::Log(orz::ERROR)
          << "read the model file:" << file << " failed!" << orz::crash;
      }
  ...
  orz::jug ModelFileInputStream::read_jug() {
    std::shared_ptr<char> buf = std::shared_ptr<char>(
      new char [m_size], std::default_delete<char []>());
    int64_t nreads = read(buf.get(), m_size);
    if (nreads <= 0) {
      orz::Log(orz::ERROR) << "read model file: " << m_file << " failed!"
                           << orz::crash;
      return orz::jug();
    }
    if (m_istxt) {
  ...
  orz::jug ModelFileInputStream::read_jug(std::shared_ptr<InputStream> in) {
    ...
    if (istxt) { omemory.write((const char *)&mask, 4); }
    ...
    while (1) {
      ...
      if (nreads > 0) {
        if (omemory.write(buf, nreads) != nreads) {
          orz::Log(orz::ERROR)
            << "write memoryoutputstream failed!" << orz::crash;
        }
      } else {
        break;
      }
  ...
  ModelFileOutputStream::ModelFileOutputStream(
    const std::string &file, bool istxt) {
    m_istxt = istxt;
