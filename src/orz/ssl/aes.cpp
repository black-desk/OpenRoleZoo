...
namespace orz
{
  ...
  class block4x4 {
    public:
      ...
      char &operator()(int row, int col) { return m_data [row * 4 + col]; }
  ...
  static unsigned char multiply(unsigned char a, unsigned char b) {
    ...
    int           i            = 0;
    for (i = 1; i < 8; i++) { temp [i] = XTIME(temp [i - 1]); }
  ...
  static char multiply(char a, char b) {
    return static_cast<char>(
      multiply(static_cast<unsigned char>(a), static_cast<unsigned char>(b)));
  }

  // a1 * b1 + a2 * b2 + a3 * b3 + a4 * b4
  static char vector4_mul(
    char a1, char a2, char a3, char a4, char b1, char b2, char b3, char b4) {
    return multiply(a1, b1) ^ multiply(a2, b2) ^ multiply(a3, b3)
         ^ multiply(a4, b4);
  }
  ...
  std::string aes128_encode_block(
...
#  warning Only support OpenSSL, please recomiple with -DORZ_WITH_OPENSSL
...
#else   // ORZ_WITH_OPENSSL
    if (key.length() != 16)
      ORZ_LOG(ERROR) << "key.length should be 16 vs. " << key.length() << crash;
    if (data.length() % AES_BLOCK_SIZE != 0)
      ORZ_LOG(ERROR) << "length of data is not a multiplier of "
                     << AES_BLOCK_SIZE << crash;
    ...
    if (mode == CBC) {
      ...
      if (iv_copy.length() != AES_BLOCK_SIZE)
        ORZ_LOG(ERROR) << "iv.length should be " << AES_BLOCK_SIZE << " vs. "
                       << iv_copy.length() << crash;
      std::memcpy(iv_buff, iv_copy.data(), AES_BLOCK_SIZE);
    }
    AES_KEY aes = { 0 };
    if (AES_set_encrypt_key(
          reinterpret_cast<const unsigned char *>(key.data()), 128, &aes)) {
  ...
  std::string aes128_decode_block(
    const std::string &key,
    CRYPTO_MODE        mode,
    const std::string &data,
    const std::string &iv) {
