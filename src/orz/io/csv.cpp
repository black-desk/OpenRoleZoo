...
namespace orz
{
  std::vector<std::vector<std::string> > CSVRead(
    ...
    std::string                            line;
    while (std::getline(file, line)) { csv.push_back(CSVParse(line, sep)); }
  ...
  void CSVWrite(
    ...
    for (auto &line: csv) { file << CSVBuild(line, sep) << std::endl; }
  ...
  std::string CSVBuildItem(const std::string &item, char sep) {
    ...
    if (item.find(sep) != std::string::npos) { need_quota = true; }
    std::regex  rgx(R"(")");
    std::string fmt(R"("")");
    std::string build_item = std::regex_replace(item, rgx, fmt);
    if (build_item.length() != item.length()) { need_quota = true; }
    std::string quota = R"(")";
    if (need_quota) { build_item = quota + build_item + quota; }
    return build_item;
  }
}  // namespace orz