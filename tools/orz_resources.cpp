...
void print_help(const orz::arg::OptionSet &options) {
  ...
  std::cout << "    "
            << "Input path can be file or folder" << std::endl;
...
int main(int argc, const char *argv []) {
  ...
  auto option_in_dir = options.add(orz::arg::STRING, { "i", "-in_dir" })
                         ...
                         ->value(orz::getcwd());
  auto option_filename = options
                           .add(orz::arg::STRING, { "n", "-fn", "-filename" })
  ...
  if (option_cpp->found()) {
    ...
    if (! option_in_dir->found()) { in_dir = orz::cut_path_tail(input_path); }
    ...
    if (! compiler.compile(
          working_root, input_path, in_dir, out_dir, header, source)) {
  ...
  if (is_abosulte_path(input_path)) {
    ...
    compiler.set_mark(
      orz::Join({ orz::getcwd(), input_path }, orz::FileSeparator()));
  }
