...
namespace orz
{
  Shotgun::Shotgun(size_t clip_size) : clip(clip_size) {
  ...
  Shotgun::~Shotgun() {
    for (int i = 0; i < static_cast<int>(clip.size()); ++i) { delete clip [i]; }
  }
  ...
  Cartridge *Shotgun::fire(const Cartridge::bullet_type &bullet) {
    if (clip.size() == 0) {
      ...
      cart->fire(
        signet,
        bullet,
        Cartridge::shell_type(std::bind(
          &Shotgun::recycling_cartridge, this, std::placeholders::_1)));
      return cart;
    }
  }

  Cartridge *Shotgun::fire(
    const Cartridge::bullet_type &bullet, const Cartridge::shell_type &shell) {
    if (clip.size() == 0) {
      bullet(0);
      return nullptr;
    } else {
  ...
  size_t Shotgun::size() const { return clip.size(); }
