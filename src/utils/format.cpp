...
namespace orz
{
  const std::string Format(const std::string &f) { return f; }

  std::vector<std::string> Split(
    const std::string &str, char ch, size_t _size) {
    ...
    while (true) {
      ...
      result.push_back(str.substr(
        left, right == std::string::npos ? std::string::npos : right - left));
  ...
  std::string::size_type FindDecollator(
  ...
  std::vector<std::string> Split(
    const std::string &str, const std::string sep, size_t _size) {
    std::vector<std::string> result;
    std::string::size_type   left = 0, right;
