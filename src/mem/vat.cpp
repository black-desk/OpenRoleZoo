...
namespace orz
{
  ...
  void Vat::free(const void *ptr) {
    ...
    if (it == m_dict.end()) { throw orz::Exception("Can not free this ptr"); }
  ...
  void Vat::reset() {
    for (auto &pair: m_dict) { m_heap.push_back(pair.second); }
    m_dict.clear();
    std::sort(
      m_heap.begin(), m_heap.end(), [](const orz::Pot &p1, const orz::Pot &p2) {
  ...
  Vat::Vat(Vat &&that) { this->swap(that); }
