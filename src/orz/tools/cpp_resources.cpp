...
namespace orz
{
  namespace resources
  {
    ...
    static const char
      * const code_source_declare_orz_resources_table_head [] = {
        "static struct orz_resources_node orz_resources_table[] = {",
      };

    static const char
      * const code_source_declare_orz_resources_table_tail [] = {
        "};",
      };

    static const char
      * const code_source_declare_orz_resources_table_size [] = {
        "static const unsigned int orz_resources_table_size =",
        "    sizeof(orz_resources_table) / sizeof(orz_resources_table[0]);",
      };

    static const char
      * const code_source_declare_orz_resources_table_find [] = {
        "static struct orz_resources_node *orz_resources_table_find(const char "
        "*key)",
        "{",
    ...
    static const char
      * const code_source_declare_orz_resources_table_empty_find [] = {
        "static struct orz_resources_node *orz_resources_table_find(const char "
        "*key)",
        "{",
        "    return NULL;",
    ...
    static inline std::ostream &write_lines(
      std::ostream &out, const char * const *lines, size_t num) {
      for (size_t i = 0; i < num; ++i) { out << lines [i] << std::endl; }
      return out;
    }

    template <size_t _Size>
    static inline std::ostream &write_lines(
      std::ostream &out, const char * const (&lines) [_Size]) {
    ...
    class cpp_working_in {
      public:
        ...
        explicit cpp_working_in(
          const std::string &path, const std::string &base = "") :
          m_backup(orz::getcwd()) {
        ...
        ~cpp_working_in() { orz::cd(m_backup); }
    ...
    static std::string trim(const std::string &line) {
      ...
      if (right >= left) { return line.substr(left, right - left + 1); }
      return "";
    }
    ...
    static std::string find_binary_files(
      ...
      if (isdir(resources_path)) {
        ...
        std::cout << "[INFO] "
                  << "Found " << binary_files.size() << " files in folder \""
                  << resources_path << "\"." << std::endl;
      } else {
        std::ifstream file_resources(resources_path);
        if (! file_resources.is_open())
          return Concat("[ERROR] ", "Can not access \"", resources_path, "\"");
        binary_files.clear();
        int         line_number = 0;
        std::string line;
        ...
        std::cout << "[INFO] "
                  << "Found " << binary_files.size() << " files in file \""
                  << resources_path << "\"." << std::endl;
      }
      return "";
    }
    ...
    static std::string open_filelist(
      ...
      try {
        for (auto &stream: filelist) {
          ...
          if (! stream->is_open())
            throw Concat(
              "[ERROR] ", "Can not access \"", this_filename, "\" in ", root);
        }
    ...
    static void close_binary_files(
      const std::vector<std::ifstream *> &filelist) {
      for (auto &stream: filelist) {
        if (stream) stream->close();
      }
    }

    static std::string open_binary_files(
      const std::string                 &working_root,
    ...
    class Schedule {
      public:
        ...
        std::map<std::ifstream *, std::vector<size_t>>
          menu;  // in file stream to witch part to list

        void write(
          std::ifstream *in, size_t count, std::vector<std::ofstream *> &out) {
      ...
      private:
        static std::vector<std::pair<size_t, size_t>> split(
          size_t count, size_t bins) {
        ...
        std::vector<std::ofstream *> find_smallest(
          std::vector<std::ofstream *> &out, int top_n) {
    ...
    static void write_main_source_file(
      ...
      source << "#include \"" << CPPCompiler::header_name
             << CPPCompiler::header_ext << "\"" << std::endl
             << std::endl;
      ...
      for (size_t i = 0; i < schedule.list.size(); ++i) {
        ...
        source << "extern const uint64_t " << block_name_header << i << "[];"
               << std::endl;
        source << "extern const uint64_t " << block_size_header << i << ";"
               << std::endl;
        source << std::endl;
      }
      ...
      for (int i = 0; i < fileinfo.size(); ++i) {
        ...
        for (auto j: list) {
          source << "    {" << block_name_header << j << ", "
                 << block_size_header << j << "}," << std::endl;
        }
      ...
      for (auto &node: nodes) {
        if (node == nullptr) {
          ...
          source << "    {\"" << node->key << "\", " << node->hash << ", "
                 << node->next << ", " << item_menu_name_header << node->value
                 << ","
                 << "},";
        }
        source << std::endl;
      ...
      write_lines(source, code_source_declare_orz_resources_table_tail)
        << std::endl;
      write_lines(source, code_source_declare_orz_resources_table_size)
        << std::endl;

      if (nodes.empty()) {
        write_lines(source, code_source_declare_orz_resources_table_empty_find)
          << std::endl;
      } else {
        write_lines(source, code_source_declare_orz_resources_table_find)
          << std::endl;
      }
    ...
    static void write_part_block(
      const char *buffer, size_t count, int id, std::ofstream &source) {
      union uint64_chunk {
          uint64_t i;
          uint8_t  c [8];
      ...
      source << "extern const uint64_t " << block_size_header << id << " = "
             << count << "UL;" << std::endl;
      source << "extern const uint64_t " << block_name_header << id << "[] = {"
             << std::endl;
    ...
    static void write_part_source_file(
      Schedule &schedule, std::ifstream &in, size_t count, Shotgun &gun) {
    ...
    static std::string write_source(
      FileNo                             &file_no,
      const std::vector<BinaryFileInfo>  &fileinfo,
      const std::vector<std::ifstream *> &filestream) {
      Schedule schedule;
      ...
      for (size_t i = 0; i < fileinfo.size(); ++i) {
        ...
        auto &stream    = *filestream [i];
        auto  data_size = filesize(stream);
        std::cout << "[Info] Compiling \"" << info.url << "\". "
                  << memory_size_string(data_size) << std::endl;
...
#define CHECK_MESSAGE(condition)                                               \
  {                                                                            \
    if (! (m_last_error_message = (condition)).empty()) return false;          \
  }
    ...
    bool CPPCompiler::compile(
      ...
      if (! isdir(local_working_root)) {
        RETURN_WITH_ERROR(
          Concat("[ERROR] Can not change working dir to ", local_working_root));
      }
      ...
      // do in local_working_root and local_output_root
      {
        ...
        CHECK_MESSAGE(open_binary_files(
          local_working_root, input_binaries, binary_stream_list));
        ...
        CHECK_MESSAGE(
          write_source(file_no, input_binaries, binary_stream_list));
      }
    ...
    bool CPPCompiler::compile(
      ...
      std::vector<BinaryFileInfo> binary_files;
      CHECK_MESSAGE(find_binary_files(
        local_working_root,
        resources_path,
        local_binary_files_root,
        binary_files))
      return compile(
        local_working_root, binary_files, local_output_root, header, source);
    }
  }  // namespace resources
}  // namespace orz