...
namespace orz
{
  ...
  binary::binary(size_t _size) { resize(_size); }

  binary::binary(const void *_buffer, size_t _size) { write(_buffer, _size); }
  ...
  size_t binary::get_pos() const { return m_index; }
  ...
  void binary::shift(int _size) { set_pos(pos::now, _size); }

  const void *binary::data() const { return m_data.get(); }

  void *binary::data() { return m_data.get(); }
  ...
  size_t binary::correct_index(int _index) {
    int c_index = std::max<int>(
      0, std::min<int>(static_cast<int>(m_size), _index));
  ...
  const void *binary::now_data() const {
    return const_cast<self *>(this)->now_data();
  }
  ...
  bool operator==(const binary &lhs, const binary &rhs) {
    ...
    for (size_t i = 0; i < long_step_size; ++i) {
      if (lhs.data<long_step_type>() [i] != rhs.data<long_step_type>() [i])
        return false;
    }
    for (size_t i = long_step_size * long_step; i < size; ++i) {
      if (lhs.data<unsigned char>() [i] != rhs.data<unsigned char>() [i])
        return false;
    }
    return true;
  }
