...
namespace orz
{
  ...
  class range {
    public:
      class iterator {
        public:
          ...
          bool operator!=(const iterator &other) const {
            if (m_step_v >= 0) return this->m_value < other.m_value;
            else
      ...
      explicit range(T end_v) :
        m_begin_v(0),
        m_end_v(end_v),
        m_step_v(end_v >= T(0) ? T(1) : (T(-1) > 0 ? 1 : T(-1))) {}

      range(T begin_v, T end_v) :
        m_begin_v(begin_v),
        m_end_v(end_v),
        m_step_v(end_v >= begin_v ? T(1) : (T(-1) > 0 ? 1 : T(-1))) {}

      range(T begin_v, T end_v, T step_v) :
        m_begin_v(begin_v), m_end_v(end_v), m_step_v(step_v) {}

      iterator begin() const { return iterator(m_begin_v, m_step_v); }

      iterator end() const { return iterator(m_end_v, m_step_v); }
    private:
      T m_begin_v;
      T m_end_v;
      T m_step_v;
    ...
    public:
      ...
      iterator begin() const {
        return iterator(
          ...
          m_end_v,
          static_cast<T>(
            std::ceil(static_cast<double>(m_end_v - m_begin_v) / m_clips)));
      }
   ...
   * @note Example input(0, 10, 3) returns [(0, 4), (4, 8), (8, 10)]
   */
  std::vector<std::pair<size_t, size_t>> lsplit_bins(
    size_t first, size_t second, size_t bins);
}  // namespace orz
