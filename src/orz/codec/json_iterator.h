    ...
    public:
      ...
      const char &operator*() const {
        if (index < 0 || index >= size)
          ORZ_LOG(ERROR) << "index out of range" << crash;
        return data [index];
      }

      const json_iterator begin() const { return json_iterator(data, size, 0); }
      ...
      bool operator==(const json_iterator &it) const {
        return self::data == it.data && self::size == it.size
            && self::index == it.index;
      }
      ...
      friend int operator-(const json_iterator &lhs, const json_iterator &rhs) {
        if (lhs.data != rhs.data)
          ORZ_LOG(ERROR) << "can not sub iterators from different init"
                         << crash;
